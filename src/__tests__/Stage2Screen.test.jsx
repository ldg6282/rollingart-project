...
import Stage2Screen from "../../app/StageScreen/Stage2Screen";
...
describe("Stage2Screen 컴포넌트", () => {
  ...
  it("컴포넌트가 올바르게 렌더링되어야 한다", () => {
    const { getByText } = render(<Stage2Screen />);
  ...
  it("메인 버튼을 누르면 확인 모달이 표시되어야 한다", () => {
    const { getByTestId, getByText } = render(<Stage2Screen />);
  ...
  it("게임을 일시정지하고 재개할 수 있어야 한다", () => {
    const { getByTestId, queryByTestId } = render(<Stage2Screen />);
  ...
  it("게임 시작 전에만 민감도를 조절할 수 있어야 한다", async () => {
    const { getByText, getByTestId, queryByTestId } = render(<Stage2Screen />);
  ...
  it("게임 오버 시 결과 모달이 표시되어야 한다", async () => {
    const { getByTestId } = render(<Stage2Screen />);
